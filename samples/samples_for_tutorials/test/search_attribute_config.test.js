...
describe('Search attribute config', () => {

  describe('Search attribute config run', () => {
    let stdout;

    before(async () => {
      stdout = execSync(`node search/search_attribute_config.js`, { cwd });
    });
  ...
  describe('Search attribute config sample result', () => {
    ...
    const request = {
      ...
      query: 'Hoodie',
      visitorId: '12345',
      filter: '(attributes.ecofriendly: ANY("recycled packaging"))'
    };
    let filterResult = [];
    ...
    it('should contain filtered values', () => {
      if (filterResult.length) {
        filterResult.forEach((item) => {
          expect(item.product, 'Object has no "attributes" property').to.have.property('attributes');
          expect(item.product.attributes, 'Object has no "ecofriendly" property').to.have.property('ecofriendly');
          expect(item.product.attributes.ecofriendly, 'Object has no "text" array').to.have.property('text');
          expect(item.product.attributes.ecofriendly.text).to.be.an('array').that.includes('recycled packaging');
        })
      } else {
        expect(filterResult).to.be.an('array').that.is.empty;
      }
